function isIdCardNo(numObj){
	...
	if (len == 15){
		...
		if (!bGoodDay){ 
			...
			var bGoodDay2 = (birthDay.getYear() == Number(arrSplit[2])) && ((birthDay.getMonth() + 1) == Number(arrSplit[3])) && (birthDay.getDate() == Number(arrSplit[4])); 
			if(bGoodDay2){
				//将15位身份证转成18位 
				//校验位按照ISO 7064:1983.MOD 11-2的规定生成，X可以认为是数字10。 
		...
		if (!bGoodDay){ 
			...
			var bGoodDay3 = (birthDay.getFullYear() == Number(arrSplit[2])) && ((birthDay.getMonth() + 1) == Number(arrSplit[3])) && (birthDay.getDate() == Number(arrSplit[4])); 
			if(bGoodDay3){
				//检验18位身份证的校验码是否正确。 
				//校验位按照ISO 7064:1983.MOD 11-2的规定生成，X可以认为是数字10。 
				var valnum; 
